...
uint64_t __attribute__ ((noinline)) magnify(int magnify_rounds)
{
  ...
  */
  ...
  asm volatile(
      ...
      // buffer stage
      // imull can be replaced by any other sequential arithmetic operations, such as add illustrated in the paper. Note that imull operations here in the buffer stage
      // will not interfere with those in the racing stage
      // In this experiment, We use "add","sub", "xor", "neg" to replace "imull" in the source code. The example of sub as follows, other operations is in our report
      "sub %%ebx, %1\n\t"
      "sub %%ebx, %2\n\t"
      "sub %%ebx, %1\n\t"
      "sub %%ebx, %2\n\t"
...
int main(int argc, char *argv[])
{
  ...
  int start = 100, end = 20000, step = 400; // default values
