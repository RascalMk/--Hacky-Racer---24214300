  ...
  for (int rep = 0; rep < setnum; rep++) {
    for (int repetition = 0; repetition < 2; repetition++)
      for (int i = 0; i < WAYSIZE; i++) {
        trash = force_read((const char *) clearSet[rep][i]+trash);
      }
  }
  for (int rep = 0; rep < setnum; rep++) {
    ...
    for (int repetition = 0; repetition < 2; repetition++)
      for (int i = 0; i < LOOPSIZE; i++)
        trash = force_read((const char *) cacheSet[rep][i]+trash);
  }

  //deliberately create l1 tlb miss
  for (int i = 0; i < 256; i++) {
    force_read((const char *) &mem[OFFSET + (i << 9)]);
  }
  ...
  for (int round = 0; round < round_num; round++) {
    for (uint8_t rep = 0; rep < setnum - 1; rep += 2) {
      ...
      for (int i = 0; i < LOOPSIZE; i++) {
        ...
        force_read((const char *) cacheSet[prefetch_b][i]);
      }
      for (int i = 0; i < WAYSIZE - 4; i++)
        force_read((const char *) clearSet[clear_b][i] + trash_a);
    }
  }
